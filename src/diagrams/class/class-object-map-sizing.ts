 ...
 * for the class diagram layout engine (./layout.ts). The header-row math
 * SHARED by object/map/json (`titleDimension`/`measureStereo`/`headerRows`/
 * `baselineOffsetFor`) now lives in the sibling ./class-object-map-header.ts
 * (split out to keep both files under the repo's 500-line-per-file cap and
 * every function under the CCN/NLOC caps) and is re-exported below so every
 * existing import site (`./class-object-map-sizing.js`) keeps working
 * unchanged. `kind:'map'` sizing lives in the sibling ./class-map-sizing.ts
 * (G3/O1 split, same 500-line-cap motivation — this file was about to
 * exceed the cap once the O1 data-row fix's doc comments landed).
 *
