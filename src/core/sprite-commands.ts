...
// ---------------------------------------------------------------------------
// SpriteRegistry — per-diagram, mirroring `SkinParam.sprites`. Split into
// `sprite-registry.ts` to keep this module under the repo's 500-line cap
// (F4-a); re-exported here so every existing importer is unaffected.
// ---------------------------------------------------------------------------
...
export function matchSpriteCommand(
  ...
  // `CommandSpriteFile`'s `jar:` form (java:108-112). Before the encoded
  // grammars: `jar:archimate/x`'s `/` can never match their DATA token, but
  // the dispatch order mirrors upstream's own command list.
