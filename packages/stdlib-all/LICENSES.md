# Licenses — @knowvah/plantuml-stdlib-all
...
| Dependency | License |
|---|---|
| `@knowvah/plantuml-stdlib` | MIT (each bundle's own license — see its `LICENSES.md`) |
| `@knowvah/plantuml-stdlib-aws` | Mixed — CC BY-ND 2.0 icon artwork + MIT glue (see its `LICENSES.md`) |
| `@knowvah/plantuml-stdlib-tupadr3` | Mixed — MIT glue + third-party icon-font artwork licenses (see its `LICENSES.md`) |

`adaml` (GPL) is deliberately **excluded** from this package and from every
other `@knowvah/plantuml-stdlib*` package (maintainer ruling,
