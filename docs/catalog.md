...
| `DisplayReaders.ts` | `getStereotypeIfAny`, `isWhite`, `toStringOf`, `contentWidth`, `cacheKeyOf`, `iteratorOf`, `equalsLike`, `createFooOf` | `Display`'s read-only derived-value methods (`getStereotypeIfAny`, `isWhite`, `toString`, `contentWidth`, `cacheKey`, the `Iterable` protocol), plus `equalsLike` and the `createFoo` factory -- split out of `Display.ts` purely to keep that f |
...
| Module | Exports | Purpose |
|---|---|---|
| `big-frame.ts` | `Dim`, `Sides`, `BigFrameStyle`, `BigFrameLayout`, `buildBigFrame` | big-frame.ts — cdd-T34 (E14 `mainframe`): `klimt/shape/BigFrame.java`, the box `core/DiagramChromeFactory.java#decorateWithFrame` wraps the WHOLE diagram body in for `mainframe TEXT` — a rounded rect with a folder-tab title cutout in its to |
...
| `EntityImageDescriptionDelegates.ts` | `hasSomeHorizontalLinkVisible`, `isThereADoubleLink`, `hasSomeHorizontalLinkDoubleDecorated`, `buildDesc`, `resolveStereotypeSprite`, `buildStereo`, `computeShieldMargins`, `hideTextOffsets`, `requireGroups` | EntityImageDescriptionDelegates — every private-instance-method body `EntityImageDescription` delegates to (`buildDesc`, `buildStereo`, the three link-scanning helpers, `computeShieldMargins`, `hideTextOffsets`, `requireGroups`). |
...
| `EntityImageDescriptionLinkScan.ts` | `hasSomeHorizontalLinkVisible`, `isThereADoubleLink`, `hasSomeHorizontalLinkDoubleDecorated` | The three link-scanning helpers `EntityImageDescription` delegates to (`hasSomeHorizontalLinkVisible`/`isThereADoubleLink`/ `hasSomeHorizontalLinkDoubleDecorated`) -- split out of `EntityImageDescriptionDelegates.ts` purely to stay under th |
...
| `layout-series-dispatch.ts` | `buildSeriesGeometries` | Per-series-type geometry dispatch (bar/line/area/scatter) for chart diagrams. |
...
| `class-body-enhanced-embeds.ts` | `EmbeddedBlockGeo`, `extractEmbeds`, `EmbedStackingContext`, `stackEmbeds` | class-body-enhanced-embeds.ts — the embedded-`{{ }}`-diagram half of `class-body-enhanced-layout.ts#buildRowsBlockRows`, split out purely to stay under this project's 500-line file cap (mirrors `class-body- enhanced-geometry.ts`'s own ident |
...
| `class-command-directives.ts` | `DIRECTIVE_COMMANDS` | Directive-style commands for the class diagram dispatch table (rules 1-3b of the original class-commands.ts COMMANDS array): comment/no-op lines, rankdir, skinparam/allowmixing no-ops, `scale` (cdd-T29: captured into `ClassDiagramAST.scale` |
...
| `class-directives-hide-cascade.ts` | `NEVER_UNLINKED`, `cascadeHidden` | cdd-T31 (A2b E5 defect b): the namespace-ancestor cascade half of `computeHiddenIds`, split out of `class-directives-removal.ts` (line cap) -- re-exported nowhere (module-private consumer only), mirroring this file family's own "split for t |
...
| `class-directives-remove-ranks.ts` | `computeRemovedRanks` | `computeRemovedRanks`, split out of `class-directives-removal.ts` (line cap, cdd-T31) -- re-exported from there so `from './class-directives- removal.js'`/`from './class-directives.js'` sites are unchanged, mirroring this file family's own |
...
| `class-dot-clusters.ts` | `nonEmptyNamespaceIds`, `buildDotClusters` | Class diagram DOT-cluster construction -- split out of ./class-dot-graph.ts (cdd-T22b, pure relocation, no logic change) to keep that file under the repo's 500-line-per-file cap, same split rationale as ./class-dot-width-floors.ts's own mov |
...
| `class-lollipop.ts` | `LOLLIPOP_SIZE`, `ASSOC_POINT_SIZE`, `LOLLIPOP_RE`, `LollipopCounter`, `LollipopOutcome`, `applyLollipop` | Interface lollipop shorthand: `Name ()-- Existing` / `Existing --() Name`. |
...
| `class-namespace-title-runs.ts` | `TITLE_LOCAL_TOP_OFFSET`, `NamespaceTitleRun`, `namespaceTitleRuns`, `NamespaceTitleLine`, `namespaceTitleLines`, `namespaceTitleWidth`, `namespaceTitleHeight`, `namespaceTitleLineBaselines`, `renderNamespaceTitleBlock`, `renderNamespaceTitleRuns`, `NamespaceTitleRenderInput`, `NamespaceTitleFallback`, `renderNamespaceTitleAuto` | class-namespace-title-runs.ts — cdd-T26: resolves a namespace/package folder-tab or rect title's `<img:>`/`<$sprite>` markup, split out of `class-namespace-shape.ts` purely to keep that file under the repo's 500-line cap (same precedent as |
...
| `class-nested-diagram-renderer.ts` | `EmbeddedDiagramDepthError`, `MAX_NESTED_DIAGRAM_DEPTH`, `RenderNestedDiagramFn`, `RenderedEmbeddedImage`, `EmbeddedRenderer`, `createNestedDiagramRenderer`, `registerClassNestedDiagramRenderer`, `getClassNestedDiagramRenderer`, `registerNestedDiagramRenderers` | class-nested-diagram-renderer.ts — CDD T27: a real `NestedDiagramRenderer` (`core/EmbeddedDiagram.ts`'s injected seam, `EmbeddedDiagram.ts:195-197`) built in the class engine's own code, per that file's own doc comment ("never inside `Embed |
...
| `class-scale-geo-body.ts` | `scaleEmbeddedBlock`, `scaleEnhancedBody`, `scaleJsonBody` | class-scale-geo-body.ts — enhanced-body/json-body scaling helpers for `class-scale-geo.ts` (cdd-T29, D4). |
| `class-scale-geo-edge.ts` | `scaleEdgeGeo` | class-scale-geo-edge.ts — `EdgeGeo` scaling helpers for `class-scale- geo.ts` (cdd-T29, D4). |
| `class-scale-geo-note.ts` | `scaleNoteGeo` | class-scale-geo-note.ts — `NoteGeo` scaling for `class-scale-geo.ts` (cdd-T29, D4). |
| `class-scale-geo-row.ts` | `scaleAtom`, `scaleRow`, `scaleGenericTag`, `scaleFolderTab`, `scaleSymbolInk`, `scaleBadgeSpriteImage`, `scaleDashArrayString` | class-scale-geo-row.ts — row/atom/chrome-level pure scaling helpers for `class-scale-geo.ts` (cdd-T29, D4). |
| `class-scale-geo.ts` | `ScaleSpec`, `scaleClassGeometry`, `ScaledTheme`, `scaleClassTheme` | class-scale-geo.ts — the `scale ...` directive for the class engine, applied at the layout->render boundary (cdd-T29, D4 -- `plans/class-divergence-drive/decisions.md`). |
...
| `class-shield-helpers.ts` | `ClipRect`, `isRowPortKind`, `clipClusterEdgeEnds`, `MagneticClusterBox`, `clusterMagneticBorder`, `clusterClipRect`, `applyClusterMagneticBorders`, `packageEndpointAnchors`, `shieldedClassifierIds` | Port/qualifier "shield" helpers for the class diagram DOT-graph builder (./class-dot-graph.ts). |
...
| `renderer-classifier-colors.ts` | `classDefaultBackground`, `resolveElementBackground`, `resolveElementFont`, `resolveElementHeaderBackground`, `resolveElementHeaderFont`, `resolveClassBackgroundByStereo`, `resolveClassBorderByStereo`, `resolveClassFontColorByStereo`, `classStereotypeBackground`, `classifierFill`, `classBorder`, `classBorderLine`, `CLASS_BORDER_STROKE_WIDTH_DEFAULT`, `classBorderStrokeWidth`, `classBorderStrokeDasharray`, `MAP_JSON_DIVIDER_STROKE_WIDTH` | Classifier-box color/border resolution: default + element-scoped background and font lookups, classifier fill, and border stroke. |
...
| `renderer-classifier-map-dividers.ts` | `mapColumnDividerEntries` | renderer-classifier-map-dividers.ts — `mapColumnDividerEntries`, split out of `renderer-classifier-box.ts` when cdd-T29 round 2's `scaleK` threading pushed that file back over the 500-line hook cap (pre- authorised split, same precedent as |
...
| `renderer-note-link-box.ts` | `LinkNoteColors`, `renderLinkNoteBox` | `note on link`'s own box — `ComponentRoseNote`, split out of `renderer-note.ts` (500-line hook cap, pre-authorised split re-export; mirrors that file's own header precedent for `renderer-note-lines.ts`/ `renderer-note-connector.ts`/`rendere |
| `renderer-note.ts` | `renderBulletAtom`, `NOTE_STROKE_WIDTH`, `noteBodyPathData`, `renderNoteText`, `renderNote`, `renderPlainNote`, `renderTipNote`, `renderOpaleNote` | Note rendering — folded-corner box + dashed connector, or the Opale zigzag-notch member-tip shape (G2/N13). |
...
| `renderer.ts` | `renderClass`, `renderClassPage` | Class diagram SVG renderer. |
...
| `link-grammar-endpoints.ts` | `EndpointShape`, `classifyEndpointShape`, `EndpointPair` | Endpoint shape classification (`CommandLinkElement.getDummy`) -- split out of `link-grammar.ts` purely to keep that file under this project's 500-line cap. |
...
| `link-grammar.ts` | `LINK_LINE_RE`, `classifyEndpointShape`, `EndpointShape`, `ParsedLink`, `parseLinkLine` | Link grammar for the descriptive-diagram parser — a faithful port of `CommandLinkElement.java` (`net.sourceforge.plantuml.descdiagram.command`), `LinkDecor.java` (`net.sourceforge.plantuml.decoration`), and `StringUtils.getQueueDirection` ( |
...
| `note-dispatch.ts` | `NoteLineOutcome`, `tryNoteHandling` | The note-command dispatch phase of the descriptive-diagram parser's line loop — split out of `parser.ts` (T8b, unknown-bucket-routing-repair) purely to keep that file under the project's 500-line cap once the "Nothing to note to" refusal (` |
...
| `parse-helpers-inline-links.ts` | `resolveInlineLinks` | Inline `[[url label]]` hyperlink-token resolution (`UrlBuilder`/ `CommandCreoleUrl`) -- split out of `parse-helpers-strings.ts` purely to stay under the project's 500-line cap. |
| `parse-helpers-strings.ts` | `resolveInlineLinks`, `StereotypeSpriteRef`, `StereotypeResult`, `ColorResult`, `LinkStereoResult`, `TagsResult`, `stripFullWrap`, `cleanId`, `resolveTextEscapes`, `resolveNewlineEscapes`, `finalizeDisplay`, `extractNodeStereotype`, `stripUrl`, `splitLeadingQuote`, `stripTrailingUrl`, `extractColor`, `extractTags`, `extractLinkStereotype` | Pure, stateless STRING-LEVEL primitives for the descriptive-diagram parser: quote/wrap stripping, id cleaning, url/link token resolution, and the stereotype/color/tag extractors that operate on a raw remainder string. |
...
| Module | Exports | Purpose |
|---|---|---|
| `ast.ts` | `ParticipantUrl`, `ParticipantType`, `Participant`, `MessageExoType`, `AbstractMessageEvent`, `MessageEvent`, `MessageExoEvent`, `NoteEvent`, `FrameEvent`, `ActivationEvent`, `DividerEvent`, `DelayEvent`, `SpaceEvent`, `NewpageEvent`, `SequenceEvent`, `BoxGroup`, `SequenceDiagramAST`, `* from ./geo.js` | AST type definitions for PlantUML sequence diagrams — the output of the parse stage. |
| `command-arrow.ts` | `returnCommand`, `ARROW_SOURCE`, `UNDRESSED_ARROW_SOURCE`, `DRESSED_ARROW_SOURCE`, `UNDRESSED_ARROW_RE`, `DRESSED_ARROW_RE`, `getInclination`, `withPart`, `arrowCommand`, `decoratedArrowCommand` | `CommandArrow` (`SequenceDiagramFactory.java:111`) — ONE upstream command, rebuilt here from the composed named groups of `sequence-arrow-regex.ts` rather than from an enumerated token table (`->`, `-->>`, `->>`, `-->`). |
...
| `renderer.ts` | `creoleRunText`, `renderSequencePage`, `renderSequence` | Sequence diagram SVG renderer. |
...
| `sequence-arrow-color.ts` | `isKnownColorToken` | `isKnownColorToken` — split out of `command-arrow.ts` to stay under the 500-line file cap; `command-arrow.ts`'s `applyStyle` is the only caller. |
| `sequence-arrow-compose.ts` | `ARROW_SOURCE`, `UNDRESSED_ARROW_SOURCE`, `DRESSED_ARROW_SOURCE`, `UNDRESSED_ARROW_RE`, `DRESSED_ARROW_RE` | The `CommandArrow` regex, fully composed -- split out of `command-arrow.ts` purely to stay under the 500-line file cap (the file's own doc comment on "what T3's skeleton leaves out" is unaffected; this is a location move, not a scope change |
...
| `state-composite-geo-shift.ts` | `shiftGeo`, `shiftTransition`, `boundingBox` | Coordinate-shifting helpers (`shiftGeo`/`resliceRegions`/`shiftTransition`) and the flat-bounding-box fallback (`boundingBox`) -- split out of `state-composite-geo.ts` purely to keep that file under this project's 500-line cap (a pure move, |
