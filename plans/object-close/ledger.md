...
| **B35** | **M40** — `jocamu-71`'s unattributed 1px canvas width | 1 | — | `jocamu-71` | **NEW, surfaced by B20.** Both mechanisms the ledger named for this fixture (M6 at B5, M19 at B20) are now fixed and it is still 1px wide (212 vs 211) on `@width`/`viewBox[2]` ONLY — every rect, text and line matches the jar exactly, and there are no non-numeric diffs left. So it is an ink-extent max-X term, on a fixture whose max-X comes from the POPULATED sibling `p2` (established at B5). Deliberately not guessed at: B34 is this session's cautionary tale for naming an ink mechanism from a coincidence. |
