...
| **B35** | **M40** — ink max-X is `max(x+w-1, x+bodyWidth)`, not a fixed per-state rule | 1 (+ corpus-wide) | — | `jocamu-71` | **MECHANISM FOUND 2026-08-11 (T7); FIX NOT ATTEMPTED — blast radius is every classifier.** A classifier's body TextBlock is drawn at the box's left edge with its OWN width (`dimFields.width`), so its ink reaches `x + dimFields.width`, while the rect reaches only `x + w - 1` (`LimitFinder#drawRectangle`). Since `EntityImageObject.calculateDimensionSlow:150-153` sets `w = max(dimFields.width, dimTitle.width + 2*xMarginCircle)`, the body reaches the box edge **only when the body is what set the width**. Jar-verified with two authored controls differing only in which term wins: body-driven `w=180` → canvas 269 (needs `x+w`); title-driven `w=201.25` → canvas 289 (needs `x+w-1`). `jocamu-71` is title-driven (`"~#1: Person"` is wider than its `tutu` body), so our fixed `x+w` overshoots by exactly 1. **This is the `+1` B5 recorded as having 'no identified drawing shape', and it retires `addRectInk`'s invisible-`UEmpty` attribution** (`UEmpty` is drawn nowhere on this path). **Brief written: `plans/b35-body-width-ink/README.md` (mission-index SI18) — a fresh context starts there, the diagnosis is complete.** Implementing it means threading the measured body width onto `ClassifierGeo` and consuming it in `class-ink-box.ts#addRectInk` — which every class/object/map/json leaf uses, so it needs its own iteration and a full class re-measure. |
