...
| 49 | `tobuka-93-jale775` | M27 | `EntityImageObject.java:151-153`; `FromSkinparamToStyle.java:241` | `class-object-map-sizing.ts:367`, `:340`; `class-layout-helpers.ts:402`, `:416-418` | fixable | size §tobuka-93 |
...
| M27 | `skinparam minClassWidth` floors an OBJECT box's width upstream; we apply `PName.MinimumWidth` only on the generic class-leaf path and only for `isLikeClass` kinds | `svek/image/EntityImageObject.java:151-153`; `style/FromSkinparamToStyle.java:241` (EMPTY `SName` varargs ⇒ matches every element) | `class-object-map-sizing.ts:367`, `:340`; `class-layout-helpers.ts:402`, `:416-418`, `:264`, `:294` | `tobuka-93-jale775` | size |
...
| B25 | **M27** — `minClassWidth` floor on objects | 1 | — | `tobuka-93` | Largest single numeric delta in the corpus (1196px) from one missing clamp. |
