...
| 17 | `jocamu-71-nuvo330` | M40 (was filed as M19) | `klimt/shape/TextBlockMarged.java:83` + `svek/image/EntityImageObject.java:150-153` | `class-ink-box.ts#addRectInk` | **FIXED (B35)** — zero-diff, ratcheted | nonnum-b §jocamu-71 |
...
| ~~M19~~ **LANDED B20** | Entity-level URL (`object … [[http://…]]`) not wrapped in the `<a>` element upstream opens around the whole entity image. **Real origin was the PARSER, not the renderer:** `class-object-commands.ts`'s `URL` fragment matched-and-DISCARDED the bracket on a stale comment ("`Classifier` has no `url` field" — untrue since G2 N15). The render path already worked; `class … [[url]]` had been fine all along | `svek/image/EntityImageObject.java:186-187`, `:211-212` | `src/diagrams/class/renderer-group.ts:78` | `jocamu-71-nuvo330` | nonnum-b |
| ~~M20~~ **LANDED B21** | `Link#getInv()` constructs a second `Link`, which consumes a tick of the shared uid counter — a left/up-directed link burns one `lnk` number that is never rendered | `CommandLinkClass.java:363-364` → `abel/Link.java:145-146`, `:135` → `net/atmp/CucaDiagram.java:745-746`, `:129` | `src/diagrams/class/renderer-uid.ts:145-233` | `sajege-04-zuce784` | nonnum-b |
| ~~M21~~ **LANDED B22** | Creole bullet-list header modelled as a width-only spacer — layout exact, but no bullet GLYPH is drawn (ellipse at order 0, rect at order ≥1) | `klimt/creole/atom/Bullet.java:58-69`; `klimt/creole/StripeStyle.java:59-62` | `src/diagrams/class/note-layout-measure.ts:365`; `src/core/klimt/creole/StripeStyle.ts:84-86` (throws) | `donoki-79-riku189` | nonnum-b |
| ~~M22~~ **BackgroundColor arm LANDED B13; generic rewrite TRACKED** | `<<stereo>>`-qualified skinparam keys modelled as a per-key allowlist; upstream splits the stereotype off ANY raw key and re-signs the style with it at +1000 priority. **Measured cost of re-mirroring generically (B13): 7 matcher rows, 13 `*ByStereo` accumulator fields, 63 consumer sites** — a redesign of the skinparam→Theme shape, genuinely large AND separable, so it stays tracked rather than done inline | `style/FromSkinparamToStyle.java:292-302`, `:396-410`, `:198`; `style/StyleLoader.java:178-186` | `src/core/skinparam-stereo-keys.ts:135-195` (no `*backgroundcolor<<…>>` matcher); `renderer-classifier-colors.ts:122-124` | `majake-62-pero492` | nonnum-b |
...
| ~~B34~~ | **M39** — flat-edge label width ignored in node separation | 3 | — | — | **RESOLVED as an ENGINE divergence and FILED 2026-08-11** — `docs/graphviz-issues/11-flat-edge-label-width-ignored-in-nodesep.md` + TRACKER line. The label-width confound is eliminated by measurement, not argument: `layoutGraph` returns the SAME 60.425 box-to-box gap for `labelWidth` 0, 29, 200 and 400, while the jar gives 50.425 / 63.425 / 232.425 for short / 29-wide / long labels — jar gap ≈ `labelWidth + 34.4`. So `svek-dot-emit.ts:44`'s `Math.round`-vs-truncate gap (0.54px) cannot be the cause; it remains a separate, real faithfulness item. Our svek DOT for `zebufu-01` is byte-identical to the oracle's. |
...
| ~~B13~~ | **M22** (BackgroundColor arm) | 1 | — | **`majake-62` FLIPPED, 1 → 0** | **DONE 2026-08-11.** Census 32 → **33/80**. Added `applyElementBackgroundColorByStereo` as an exact mirror of `babcfa94`'s `applyElementFontSizeByStereo`, including its ordering contract (runs after `STEREO_KEY_MATCHERS`, since its `\w+` prefix also matches `statebackgroundcolor<<X>>`). Retired the stale "SEPARATE, larger, deferred" note in `renderer-classifier-colors.ts`. **This is knowingly a patch on the divergence M22 names**, not the re-mirror — the generic form is measured above and tracked. |
...
| ~~B20~~ | **M19** — entity `<a>` URL wrapper | 1 | — | `jocamu-71` 3 → 2, does NOT flip | **DONE 2026-08-11.** The `childCount` 4-vs-1 is gone: the body now merges into one `<a>` run exactly as the jar emits it. Fix was one capture group in the object-declaration grammar plus `classifier.url = parseUrlBracket(...)` — the entire render path (`ClassifierGeo.url` → `wrapClassifierBody`'s per-primitive `?? geo.url` default) already existed and needed no change. A **1px width residual** remains and is NOT attributed — see B35. |
| ~~B35~~ | **M40** — ink max-X is `max(x+w-1, x+bodyInkWidth)`, not a fixed `x+w` | 1 | — | `jocamu-71` | **DONE 2026-08-11.** Landed narrower than the brief predicted, and with a CORRECTED mechanism. The `+1` is a real `UEmpty`, but sized to the BODY, not the box: `TextBlockMarged#drawU`'s `ug.draw(UEmpty.create(dim))` (`klimt/shape/TextBlockMarged.java:83`) is the ONLY `UEmpty` draw site in the whole upstream tree, `dim` is that block's own, and `LimitFinder#drawEmpty:159-162` bounds it with no inset. Every body compartment is wrapped in one (`BodyEnhancedAbstract#decorate:106-118`, `MethodsOrFieldsArea#asBlockMemberImpl:83-86`), so the reservation clears the rect's `-1` corner only when the body set the width. **This retires BOTH prior attributions**: G2 N5's full-box `UEmpty` (right shape, wrong size) and T7's "`UEmpty` is drawn nowhere" (it read the entity images and missed `TextBlockMarged`). **The brief's "corpus-wide, expect class movement" scope claim is FALSE**: `HeaderLayout#drawU:89-109` centers the class name in `suppWith = width - circleW - widthStereoAndName - genericW`, which is exactly 0 when the header drove the width, so a class name's own `UEmpty` lands on `x+w` and the fixed rule was already right there — which is why all 317 byte-exact class goldens hold unchanged. An object header cannot do this (`PlacementStrategyY1Y2:59`'s strict `x = (width - blockWidth)/2` leaves `xMarginCircle`=5px clear each side), so the conditional rule is object-family-scoped; `map`/`json` left unmeasured (different body classes, no jar control). **G3/O2's `addRectInkEmptyBody` DELETED** — proven a strict special case (`TextBlockUtils.empty(0,0)` ⇒ `bodyInkWidth` 0 ⇒ the same `(x+w-1, y+h)` corner); disabling it leaves the census zero-diff SET byte-identical, while disabling the empty-but-SHOWN sibling drops it 35→29 (that one survives on max-Y alone). Census 34 → 35; all five DOT counts and both golden ratchets unmoved. |
| ~~B21~~ | **M20** — `Link#getInv()` uid tick | 1 | — | **`sajege-04` FLIPPED, 2 → 0** | **DONE 2026-08-11.** Census 31 → **32/80**, nothing else moved. Reused the EXISTING `Relationship.phantomSlot` rather than adding a field: a discarded `Link` ctor burning a `cpt1` slot immediately before the rendered edge is exactly what that flag already models (its other producer is the couple machinery's synthetic default link), so the tested phantom-rank path in `renderer-uid.ts` needed no change at all. |
| ~~B22~~ | **M21** — creole `Bullet` glyph atom | 1 | — | **`donoki-79` FLIPPED, 3 → 0** | **DONE 2026-08-11.** Census 33 → **34/80**. Scope was NARROWER than filed: the object body's `*` rows were already byte-correct (they are `VisibilityModifier.IE_MANDATORY` glyphs, `rx=3` WITH stroke — `SIMPLE_LINE` skips the bullet pattern entirely). Only the NOTE's creole bullets were missing. Vertical placement is DERIVED, not fitted: `Sea.java:72-79` lays each atom at `-height + altitude`, which is **-10 at both bullet orders** (`5-5`, `3-7`), so the top is `lineTop + lineHeight - 10`. |
...
| ~~B32~~ | **M41** — `taillabel`/`headlabel` centres diverge from graphviz | 1 | — | — | **RESOLVED as an ENGINE divergence and FILED 2026-08-11 (T6)** — `docs/graphviz-issues/12-port-label-placement-near-head-node.md` + TRACKER line. Reproduced on a 2-node/1-edge graph whose emitted DOT is byte-identical to the jar's: graphviz clears both port labels ~14.5px from their node edge; graphviz-ts places the tail at 12.80 and the **head at 2.96**. Deltas 1.784 (tail) and 11.439 (head) reproduce UNCHANGED on `tobuka-93`'s 9-edge graph, so one mechanism. Our centre→baseline conversion is falsified as the cause: it is uniform (+10.611 for both labels, x offset exactly half each label's own width), and a conversion error would shift both equally rather than in opposite directions. NOT M2/M34 — `tobuka-93` is M2's own named control. |
...
| ~~B31~~ | **M36** — the DOT structural comparator cannot see edge direction | gate-wide | — | — | **DONE 2026-08-11, maintainer-approved.** Added `directionOk` to `tests/oracle/svek-dot.ts#structurallyEqual`: a sorted `in:out` degree multiset, id-agnostic for the same reason `degreeSequence` is. Re-baselined the three counts it moves — object 74 → **73/80**, class 689 → **661/711**, state 267 → **264/267**; component 262 and usecase 93 unmoved. The 32 fixtures it surfaced are pre-existing defects, NOT regressions, quarantined per the `port-backlog.json` precedent B0 set — see B33. |
| **B33** | **M38** — 32 fixtures emit at least one edge in the wrong direction | 32 | — | — | **NEW, surfaced by B31.** class 28 · state 3 · object 1, listed in `oracle/goldens/<type>/direction-backlog.json`. Each is pinned to fail `directionOk` and NOTHING else, so every other structural check stays live and the list is shrink-only. **Cause not yet attributed** — this is an audit item, not a fix item. B6 fixed M7's decor-driven swap, so these are something else. Sampled `class/befasi-62-vimu310`: the candidate's out-degrees collapse into a hub (`5:0`, `7:0`) where the oracle has none and its maxima are `3:4`/`1:3`, so at least some are edges we emit from a single source that upstream emits per-pair — note connectors and inline `extends` are the two builders that carry no `idEntity1FullId` pair and fall back to the `parentIsLinkEntity1` rule. |
