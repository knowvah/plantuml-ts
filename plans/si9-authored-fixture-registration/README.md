...
1. **`tests/oracle/svg-conformance/PARITY-SVG.md` is 3 rows stale.** It is
   committed, generated by `svg-parity-dashboard.ts` from `parity.json`, and
   asserted by no test. Outside every task's write-set, so it was reported
   rather than silently regenerated.
2. **The 265 `component` parity rows still date from 2026-07-15** while
   `generatedAt` now reads 2026-07-31. ADR-3 scoped regeneration to `usecase`;
   a component re-survey will surface drift of the same kind T2 found.
