# T3 — bisect `class/tunelu-64-xica833`
...
cached `svek-1.dot` in `test-results/dot-cache/class/tunelu-64-xica833/`. That cache is
fixed data, independent of `src/` — which is what makes this bisect sound.

`tunelu-64-xica833` reports `dotEqual=false` today; its pin claims `true`. T0 has
...
- `.agent-notes/bisect-doteq-tunelu-64-xica833.md`
...
One commit: `docs(T3): bisect tunelu-64-xica833 to its culprit`
