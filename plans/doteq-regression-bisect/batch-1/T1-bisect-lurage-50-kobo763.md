# T1 — bisect `state/lurage-50-kobo763`
...
cached `svek-1.dot` in `test-results/dot-cache/state/lurage-50-kobo763/`. That cache is
fixed data, independent of `src/` — which is what makes this bisect sound.

`lurage-50-kobo763` reports `dotEqual=false` today; its pin claims `true`. T0 has
...
- `.agent-notes/bisect-doteq-lurage-50-kobo763.md`
...
One commit: `docs(T1): bisect lurage-50-kobo763 to its culprit`
