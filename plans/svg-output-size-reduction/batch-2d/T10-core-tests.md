# T10 — Core / emitter unit tests

**Agent:** test-automator · **Depends on:** T3–T9 · **Commit:** `test(T10): update core emitter expectations for reduced SVG form`

## Write-set

`tests/unit/core/**` — the klimt drawing/svg tests, `svg.test.ts`,
`number-format`/`svg-format` tests, openiconic tests.

## Read-set

- `src/core/svg-format.ts`, `src/core/svg.ts`,
...
1. Given every suite under `tests/unit/core/`, when run, then all pass.
2. Given a test that asserted `toFixed(5)` opacity or a 4-decimal literal,
   then it asserts the new form with the same intent — not a weakened one.
3. Given coverage, then the 90/90/90 thresholds still hold.
