# Batch 5 — G20a declaration order (serial, LAST, revert-on-net-growth)
...
|---|---|---|---|---|---|
| T5 | Push pseudo nodes in jar creation order in both composite passes | general-purpose (opus) | `src/diagrams/state/state-composite-autonom.ts`, `src/diagrams/state/state-composite-concurrent.ts`, `src/diagrams/state/state-composite-pseudo.ts` (only if the ordering helper itself must change), their unit tests | T4 | [ ] |
