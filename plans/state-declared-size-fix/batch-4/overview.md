...
|---|---|---|---|---|---|
| T10 | F9 — G11 `--` vs `\|\|` concurrent-separator orientation | typescript-pro | `src/diagrams/state/ast.ts`, `state-commands.ts`, `state-composite-sizing.ts`, `state-composite-concurrent.ts`, `tests/unit/state/state-concurrent-composite.test.ts`, ratchet entries | T8 | [x] |
| T11 | F10 — G13 transition-label ink-box position (diagnose first; fix only if the mechanism closes) | typescript-pro | `src/diagrams/state/layout-ink-extent.ts`, `state-composite-edge-label.ts`, `tests/unit/state/layout-ink-extent.test.ts`, ratchet entries, `plans/state-declared-size-fix/findings/G13.md` if it stays open | T9 | [x] |
