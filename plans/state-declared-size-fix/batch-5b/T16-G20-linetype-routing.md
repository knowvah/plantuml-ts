# T16 — D5: G20 composite ink under linetype polyline/ortho (kejabo-83, pavuzo-79)
...
## Task
SI28 (`skinparam-style.md`): leaf rows exact; the composite differs +0.75 (kejabo, `label=` inline path via `state-composite-pass.ts:250`) vs −2.46 (pavuzo, `xlabel=` under ortho via `state-dot-graph.ts:238`); Java `SvekEdge.java:433-437`. Two mechanisms, not one. nextStep: extract each inner scope's DOT fragment (from `setLayoutInputObserver`), route it through dot-engine and the oracle graphviz, and diff the routed points / xlabel anchor that feed the ink box; decide engine-side vs ours-side per fixture.

## Write-set
`plans/state-declared-size-fix/findings/G20-linetype-routing.md` ONLY.
