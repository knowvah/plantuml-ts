...
### The SVG-sprite ink gap — **CLOSED 2026-07-31** by mission `svg-sprite-nanoparser`

> **Status: closed.** The diagnosis below (the third, jar-verified version)
> was correct and was implemented in full. Branch: `main`, 13 tasks,
> commits `6a5d3e7d`…`bfb876bf`. See
> `plans/svg-sprite-nanoparser/` for the brief and decision journal.
>
> **What was achieved.** Upstream's two independent channels are restored
> structurally, not numerically. `SvgNanoParser`, `SvgPath`, `ColorResolver`
> and `UGraphicWithScale` are ported in full (982 Java lines);
> `AtomImageResolver` carries a `drawable` variant whose primitives hold
> draw-time shape, translate and paint, while `width`/`height` remain the
> declared box in both variants; `fitToInk` is retired and the sizer now runs
> the same decomposition the renderer does, so `Footprint` observes real
> per-path corners. All three authored sprite fixtures match the jar with
> **zero diffs** under `compareSvg(…, 'deterministic')` and are DOT-equal.
> 456 files / 11,152 tests; 389 SVG goldens unchanged; **320/351 (91.2%),
> widened 0**.
>
> **What was NOT achieved, stated as plainly as what was.**
> 1. **The conformant count did not move, and was never going to.**
>    `bootstrap-0` and `ruziru-69-xixo434` were verified against `HEAD` to be
>    ALREADY conformant at delta 0 before the retirement task ran —
>    `fitToInk` itself was what made them so. The 0.029321in figure this
>    entry names had already been closed by the very hack the mission
>    retired. **This mission's value is architectural, not numeric.** A
>    reader who measures 320/351 before and after and concludes nothing
>    happened would be wrong, but understandably so.
> 2. **The three sprite fixtures are not ratcheted in.** They pass both
>    substantive bars but AC3 eligibility needs a `parity.json` entry, and
>    `parity.json` is generated by `svg-parity-survey.ts` over the local
>    `test-results/dot-cache` corpus, which authored fixtures never enter.
>    A registration gap, not a quality one. Filed as SI9.
> 3. **The class-engine coupling is untouched** — see the block at the end of
>    this entry. Still open, now inherited by whoever next touches
>    `leaf-sizing.ts`'s resolver.
> 4. **`<circle>`/`<text>` sprite ink was nearly lost twice** — first to a
>    `UPath[]` type, then to a paint-dropping collector. Both were caught by
>    measurement, not by the gates. See item 5.
> 5. **No quality gate could see the mission's worst regression.** Emitting
>    `drawable` initially made sprites render as NOTHING, and `npm test`,
>    all 389 goldens and the size-delta script stayed green throughout.
>    Sizing was unaffected and no ratcheted golden contains a sprite. Only
>    the three diagnostic fixtures caught it. **The sprite corpus has no
>    regression guard until SI9 lands.**

### The original diagnosis (retained; still the authoritative mechanism)
