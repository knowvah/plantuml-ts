...
in classdiagram/ + net/atmp/CucaDiagram). Fix at origin. dot-engine
...
2. ~~`svg/g/g/path/@d`~~ — **N5 landed** (bezier `C` commands, was `L`
   ...
   dot-engine-vs-real-graphviz ROUTING divergence is genuinely
...
11. **~7-8px multi-component/namespace-cluster position/height offsets**
    (unchanged from N7, may overlap the dot-engine coordinate-assignment
...
17. **dot-engine coordinate-assignment ~7px offset** (unchanged since N8) —
    OUT OF SCOPE per CLAUDE.md (dot-engine pinned .tgz); candidate for an
    upstream dot-engine issue.
...
15. **`sadamo-18-siva346`** pathological stress fixture (unchanged since
    N9).
16. **dot-engine coordinate-assignment ~7px offset** (unchanged since N8)
...
18. **`sadamo-18-siva346`** pathological stress fixture (unchanged since
    N9-N10).
19. **dot-engine coordinate-assignment offset** (unchanged since N8) — OUT
...
7. **~2px uniform position offset across UNCONNECTED sibling classifiers**
   ...
   `mujopi-06-lusi222`) -- likely dot-engine packing margin, needs a
...
10. Every item unchanged from N14's own queue not superseded above
    ...
    `sadamo-18-siva346`, dot-engine coordinate offset, N12's single-
...
2. **Title-driven package width floor** (`pixexi-81-sete111`, `skinparam
   ...
   floor in `buildNamespaceGeos`, plus verifying dot-engine's own
...
1. **Anchor-in-cluster footprint, full jar parity** (`bajotu-30-soku184`/
   ...
   full parity additionally needs a dot-engine rank-assignment fix
   ...
   effect). Candidate for an upstream dot-engine issue.
2. **Title-driven package width floor + centering** (`pixexi-81-sete111`)
   ...
   `node_modules/dot-engine/dist/api/builder.d.ts`); a pure post-layout
...
entirely absent) — the remaining gap is RENAMED/NARROWED to a dot-engine
...
4. Every item unchanged from N18's own queue not superseded above
   ...
   suppression, `skinparam mode dark`, `sadamo-18-siva346`, dot-engine
   coordinate offset, N12's single-fixture unsurveyed residuals, anchor-in-
   cluster footprint (dot-engine-blocked), title-driven package width
   floor (dot-engine-blocked), strictuml classifier-spot-badge
...
2. Every item unchanged from N19's own queue item 4 not superseded above
   ...
   9 repeat-coupling fixtures — a strong future ROI target if dot-engine
   is ever brought in scope), N12's single-fixture unsurveyed residuals,
   anchor-in-cluster footprint (dot-engine-blocked), title-driven package
   width floor (dot-engine-blocked), strictuml classifier-spot-badge
   suppression, package/namespace stereotype -> `PackageStyle` dispatch,
   lollipop half-circle socket shape (ZERO corpus reach, confirmed N20 —
   low priority even if dot-engine is ever unblocked), file-size-cap
...
dot-engine; repeat-coupling burn order (was 9/718, named since N9) —
structurally complete, blocked only by dot-engine. Both narrowed OUT of
...
10. Every item unchanged from N20's own queue item 2 not superseded above
    ...
    mode dark`, `sadamo-18-siva346`, dot-engine coordinate offset (sole
    blocker on 13 lollipop + 9 repeat-coupling fixtures), N12's
    single-fixture unsurveyed residuals, anchor-in-cluster footprint
    (dot-engine-blocked), title-driven package width floor
    (dot-engine-blocked), strictuml classifier-spot-badge suppression,
...
5. Every item unchanged from N21's own queue not superseded above
   ...
   dot-engine coordinate offset, anchor-in-cluster footprint, title-driven
...
11. **`sasito-46-padu855`'s space-before-colon bug** (a member's own
    ...
    traced).
11a. **`dot-engine` builder API has no fixed-size/HTML-table label
    override** (NEWLY DISCOVERED N25, `node_modules/dot-engine/src/api/
    ...
    placements, since `dot-engine`'s internal `xladjust` search sizes the
    ...
    needs either a `dot-engine` change (OUT OF SCOPE) or a second,
...
(`xlabels.c` external-label force-search) reused via `dot-engine`'s own
...
12. **Bare (non-"empty") global `hide methods`/`hide fields`** (NEWLY
    ...
    priority).
13. **`dot-engine` fixed-size (HTML `FIXEDSIZE`) label-sizing API gap**
    (unchanged since N25, item 11a there) — deferred, needs a
    `dot-engine` change or a second DOT-text-only layout pass.
14. Every item unchanged from N22's own queue not superseded above
    ...
    `sadamo-18-siva346`, dot-engine coordinate offset, anchor-in-cluster
...
16. Every item unchanged from N26's own queue item 14 not superseded
    ...
    height`, `[hidden]` suppression, `sadamo-18-siva346`, dot-engine
...
1. **`class-dot-graph.ts#buildDotEdges`'s DOT tail/head derivation**
   ...
   `dot-engine` fed jar's exact cached DOT text (`duruga-39-lani451`) —
   NOT a dot-engine engine divergence (third consecutive confirmation
...
- **gvts-genuine** (confirmed engine-level, OUT OF SCOPE per CLAUDE.md, no
  `dot-engine` API surface exists for these): the dominant `svg/g/g/
...
- **Small universal residual** (both RECT and FOLDER, NOT chased N60):
  ...
  classifier placement inside it) — suspected dot-engine-vs-real-graphviz
  margin/nodesep default divergence, which per this mission's CLAUDE.md is
  OUT OF SCOPE (dot-engine internals). Blocks all 3 named fixtures from
...
Geometry extraction from dot-engine stays on the SVG-text scan
...
graph/layout data mid-mission; a structured dot-engine API cutover
...
1. gvts-blocked ~288 (67.6%) — five verified dot-engine items
...
**Reopeners.** A dot-engine release addressing item 1's first two
