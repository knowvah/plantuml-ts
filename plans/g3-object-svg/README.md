...
|---|---|---|
| `gvts-blocked` — edge-spline `path/@d` (dot-engine numeric divergence, pending ADR-1 dot-engine cutover) | 40 fixtures | 40 |
...
1. **`gvts-blocked` (46/80 combined)** — resolved only by the dot-engine
   ADR-1 cutover (dot-engine numeric-layout parity), out of scope for
