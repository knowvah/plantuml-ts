...
`findings/PARTITION.md#composite-b` (your slice — authoritative over the preview),
`~/.claude/rules/diagnosis.md`, CLAUDE.md "READ THE JAVA FIRST".

## Slice (preview at d9f0ddda; T0's PARTITION.md wins)
`pacami-67-dafe414`, `pebepi-32-cati486`, `rovese-43-tadu368`, `taxile-56-goca422`, `tigibi-80-zidi137`, `tofezi-64-koda860`, `vixobo-14-jole910`, `xojudi-20-keco020`, `zizemo-86-gisa766`, `zoriza-41-rege543`

## Hints (verify, never inherit — ADR-4)
Same family as T1 (`composite-a`); coordinate on `sharedCauseWith` by naming T1's slugs where the mechanism matches. zoriza-41 (−41 px h) and zizemo-86 (−21) look like the T1 −21/−39 rows.
...
## Write-set
`plans/state-declared-size-diagnosis/findings/composite-b.md` ONLY.
...
- Given `git status`, then only `findings/composite-b.md` is new; `scripts_scratch/` is gone.
