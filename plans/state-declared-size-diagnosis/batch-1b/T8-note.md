...
`findings/PARTITION.md#note` (your slice — authoritative over the preview),
`~/.claude/rules/diagnosis.md`, CLAUDE.md "READ THE JAVA FIRST".

## Slice (preview at d9f0ddda; T0's PARTITION.md wins)
`fatupo-62-bemu777`, `tumaba-64-tosu281`, `xeziki-47-zomo866`

## Hints (verify, never inherit — ADR-4)
notes attached to states/transitions. xeziki-47 158 px, fatupo-62 37, tumaba-64 13. Java: `svek/image/EntityImageNote.java`, `EntityImageNoteLink.java` (SI27 T6 unified `measureLinkNoteDim` — read `src/core/svek/image/EntityImageNoteLink.ts`), `statediagram/command/CommandCreateNote*`.
...
## Write-set
`plans/state-declared-size-diagnosis/findings/note.md` ONLY.
...
- Given `git status`, then only `findings/note.md` is new; `scripts_scratch/` is gone.
