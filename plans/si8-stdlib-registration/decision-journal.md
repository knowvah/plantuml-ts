...
| Date | Task | Decision | Rationale |
|---|---|---|---|
| 2026-07-31 | T7 | The deferred per-resource work is tracked as **SI11**, its own row | Criterion 2 requires a row a future mission can pick up, not a sentence inside SI8's prose. Carries ADR-2's measurements verbatim (1.06 MB / 7.93 MB / 19.54 MB; 6,849 tupadr3 files, 29,101 across assets) and both options ADR-2 recorded WITHOUT deciding — per-resource static modules vs runtime HTTP fetch reusing `IncludeFetcher` |
| 2026-07-31 | T7 | The goldens README section was **amended with a dated subsection**, not rewritten | Matches how SI9 amended `svg-sprite-nanoparser`'s ADR-5. The 2026-07-31 ratcheting measurements stay exactly as taken (they were true when taken — SI9's ADR-4), and the si8 amendment records what changed underneath them. Also carries the validated capture command and the `-nometadata` warning, so the next person re-capturing does not repeat that |
| 2026-07-31 | T7 | Historical rows were not retro-edited | The SI8 row's original framing is **corrected in place with the correction labelled**, rather than silently replaced, so the misreading it invited is on the record. SI5b's and SI9's dated numbers untouched |
| 2026-07-31 | Batch 5 | Gates: **all green, unmoved** | `npm test` 459 files / 11219 tests; typecheck, lint, build exit 0; size-deltas 320/351 widened 0. Documentation-only, and nothing moved — which is the expected result |
