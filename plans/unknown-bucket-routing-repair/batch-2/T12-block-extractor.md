# T12 — fix the block-extractor seam
...
`fix-candidate` rows whose `seam` is `block-extractor` — the orchestrator lists them in
this file's **Fixtures** section below, with each row's diagnosis
(`diagnosis/TN.md`) naming the upstream Command and the mechanism. Expected inputs: candidate-set or probe defects any diagnosis attributed to the block extractor (the 20-line detection window, `@start` tag parsing). Measured over the whole tree by definition.
...
## Write-set
`src/core/block-extractor.ts`, its unit tests, and ONLY the ledger fragment files listed in
`batch-2/overview.md` for T12.
...
## Commit
`fix(ubrr-T12): <mechanism summary> in the block-extractor seam` — body lists the fixtures
