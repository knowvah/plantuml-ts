...
`fix-candidate` rows whose `seam` is `description` — the orchestrator lists them in
this file's **Fixtures** section below, with each row's diagnosis
(`diagnosis/TN.md`) naming the upstream Command and the mechanism. Expected inputs: over-claims of ACTIVITY/CLASS sources by the description probe (T2, T4) and description refusals (T5). `descriptive-keywords.ts` is shared with the dispatch path — a change there is measured over the whole tree.
...
## Write-set
`src/diagrams/description/** and src/core/descriptive-keywords.ts`, its unit tests, and ONLY the ledger fragment files listed in
`batch-2/overview.md` for T8.
...
## Commit
`fix(ubrr-T8): <mechanism summary> in the description seam` — body lists the fixtures
