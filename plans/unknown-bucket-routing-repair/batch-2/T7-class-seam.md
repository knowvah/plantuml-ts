...
`fix-candidate` rows whose `seam` is `class` — the orchestrator lists them in
this file's **Fixtures** section below, with each row's diagnosis
(`diagnosis/TN.md`) naming the upstream Command and the mechanism. Expected inputs: the `Class-visibility-*` family (T3) and the CLASS→DESCRIPTION refusals (T4) where class refused a line upstream's `ClassDiagramFactory` accepts.
...
## Write-set
`src/diagrams/class/**`, its unit tests, and ONLY the ledger fragment files listed in
`batch-2/overview.md` for T7.
...
## Commit
`fix(ubrr-T7): <mechanism summary> in the class seam` — body lists the fixtures
