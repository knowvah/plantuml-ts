...
`fix-candidate` rows whose `seam` is `state` — the orchestrator lists them in
this file's **Fixtures** section below, with each row's diagnosis
(`diagnosis/TN.md`) naming the upstream Command and the mechanism. Expected inputs: the state parser accepting DESCRIPTION/ACTIVITY/NONE sources it should refuse (T2, T5, T6) — strictness, D2(b).
...
## Write-set
`src/diagrams/state/**`, its unit tests, and ONLY the ledger fragment files listed in
`batch-2/overview.md` for T9.
...
## Commit
`fix(ubrr-T9): <mechanism summary> in the state seam` — body lists the fixtures
