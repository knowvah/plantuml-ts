...
`fix-candidate` rows whose `seam` is `sequence` — the orchestrator lists them in
this file's **Fixtures** section below, with each row's diagnosis
(`diagnosis/TN.md`) naming the upstream Command and the mechanism. Expected inputs: sequence over-claiming CLASS/DESCRIPTION sources (T4, T5) and sequence refusals (T5, T6). The sequence engine is the most-ratcheted one in the repo — run `tests/oracle/svg-conformance/sequence*.test.ts` too before returning.
...
## Write-set
`src/diagrams/sequence/**`, its unit tests, and ONLY the ledger fragment files listed in
`batch-2/overview.md` for T11.
...
## Commit
`fix(ubrr-T11): <mechanism summary> in the sequence seam` — body lists the fixtures
