...
fixtures and this port another; `cohorts/T3.tsv` lists them with jar type,
our type, whether we errored and which engine the error page names.
Known so far: the 16 `Class-visibility-N` fixtures are jar-CLASS sources this port's class parser REFUSES (error page names class); the other 3 refused with the page naming state, description or nothing. Upstream: `ClassDiagramFactory.java` command list; visibility modifiers live in `CommandCreateClass`/member parsing (`classdiagram/command/`).

## Task
For EVERY row of `cohorts/T3.tsv`: (1) read the source
...
(re-measure; do not assume from the name). Expected shape: one or two missing member-syntax forms in `src/diagrams/class/class-member-parser.ts` — D2(c) candidates if each is one Command; otherwise pin naming the Command.

Write `diagnosis/T3.md`: a table mechanism → count → upstream cite → our
`file:line` → verdict → seam, then one paragraph per mechanism with the
exemplar's refusing line. Write the fragment
`tests/oracle/svg-conformance/unknown-ledger/T3-*.json` (name per the batch
table) with one D4 row per slug, `task: "T3"`.

## Write-set
The two files named in `batch-1/overview.md`. Nothing under `src/`.

## Read-set
`plans/unknown-bucket-routing-repair/{decisions.md,cohorts/T3.tsv,
...
1. Given the cohort, when the fragment is parsed, then every slug in
   `cohorts/T3.tsv` has exactly one row and no other slug appears.
...
## Commit
`docs(ubrr-T3): diagnose the class-jar refusals cohort (19) cohort`
