# T5 — diagnosis: description-, sequence- and state-jar cohort (18)
...
fixtures and this port another; `cohorts/T5.tsv` lists them with jar type,
our type, whether we errored and which engine the error page names.
Known so far: jar DESCRIPTION → ours STATE 5, CLASS 3, SEQUENCE 1, refused 3; jar SEQUENCE → refused by our sequence parser 5 (`covipu-77-jimo313`, `teoz-ng-001-17`); jar STATE → ours CLASS 1 (`zolaza-45-sepi570`). DESCRIPTION→CLASS/SEQUENCE are over-claims by engines registered before description; DESCRIPTION→STATE means description refused and state accepted; the sequence refusals are missing sequence Commands (the `sequence-command-coverage` mission's known-gap style applies).

## Task
For EVERY row of `cohorts/T5.tsv`: (1) read the source
...
(re-measure; do not assume from the name). Name the over-claiming probe line for each over-claim (`descriptive-keywords.ts`, the engine's `index.ts`) and the refusing line + upstream Command for each refusal.

Write `diagnosis/T5.md`: a table mechanism → count → upstream cite → our
`file:line` → verdict → seam, then one paragraph per mechanism with the
exemplar's refusing line. Write the fragment
`tests/oracle/svg-conformance/unknown-ledger/T5-*.json` (name per the batch
table) with one D4 row per slug, `task: "T5"`.

## Write-set
The two files named in `batch-1/overview.md`. Nothing under `src/`.

## Read-set
`plans/unknown-bucket-routing-repair/{decisions.md,cohorts/T5.tsv,
...
1. Given the cohort, when the fragment is parsed, then every slug in
   `cohorts/T5.tsv` has exactly one row and no other slug appears.
...
## Commit
`docs(ubrr-T5): diagnose the description-, sequence- and state-jar cohort (18) cohort`
