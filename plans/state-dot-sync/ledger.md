...
|------|-----------|--------|
| fotuje-06-fifa085 | ~~`dot-engine-crash`~~ **RESOLVED 2026-07-14 by dot-engine@0.1.26071415** (same root cause as plantuml-ts xusuxe-62: labeled cross-cluster-boundary edge; fixture now 267/267 EQUAL, golden-pinned with a 3.24in size-backlog entry) — original entry: | Our state engine's DOT input is byte-structurally correct (graph #0 verified EQUAL against the oracle), but `dot-engine@0.1.26071122`'s `render()` throws `RenderError: Cannot read properties of undefined (reading 'info')` on this fixture's 5-level nested-cluster + labeled cross-cluster-boundary edge shape, aborting before the two flattened-wrapper re-entry passes fire (oracle 3 graphs, ours 1). Minimal reproduction isolated (removing the label from one specific cross-cluster edge, or certain other edges, avoids the crash). Per the maintainer's standing rule, layout defects downstream of a matching DOT are filed against dot-engine, not worked around here — **recommend filing the minimal repro as a dot-engine issue** (repro steps in `plans/state-dot-sync/decision-journal.md`, iter 18). |
...
raised by the dot-engine npm swap (~0.19px, journaled); one by design
