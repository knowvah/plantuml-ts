...
|---|---|---|---|---|---|
| T1 | `DotInputGraph.linetype` + shared `dotSplinesAttrs` helper | sonnet | `src/core/graph-layout.types.ts`, `src/core/dot-splines.ts` (new) + its test | T0 | [ ] |
| T2 | `applyGraphAttrs` consumes the helper (layout side) | sonnet | `src/core/graph-layout-build.ts` + its test | T1 | [ ] |
| T3 | `graphAttrLines` consumes the helper (emitter side) | sonnet | `src/core/svek-dot-emit.ts` + its test | T1 | [ ] |
