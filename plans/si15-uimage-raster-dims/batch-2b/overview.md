...
|----|-------------|-------|--------|------------|------|
| T6 | Populate raster dims at the sizing resolver; correct the formula at both resolvers; re-measure | typescript-pro | `src/diagrams/description/leaf-sizing-entity.ts`, `src/diagrams/description/render-atoms.ts`, `tests/unit/core/svek/footprint-raster-dims.test.ts`, `tests/unit/creole-img-render.test.ts`, `tests/oracle/svg-conformance/class-usecase-actor.test.ts` (only if pins move) | T1, T3, T4 | [ ] |
