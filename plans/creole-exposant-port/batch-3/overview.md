...
|---|---|---|---|---|---|
| T4 | Class: member atoms + notes measured with muted size and Sea `dy`; renderers draw the SAME | typescript-pro | `src/diagrams/class/class-member-creole.ts`, `renderer-classifier-rows.ts`, `renderer-note.ts`, `note-layout-measure-rows.ts`, class unit tests, class fixture ratchet/goldens | T3 | [ ] |
