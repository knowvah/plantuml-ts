# `svg-yaml` — SVG-conformance goldens for `@startyaml`
...
**No layout of its own.** `src/diagrams/yaml/index.ts` parses with `parseYaml` and then imports `layoutJson`/`renderJson` directly. Every json layout change is transitively this type's.

## Why there is no `parity-yaml.json`, and no AC3
...
`@startyaml` diagram produces the SVG and no `.dot` file at all.
