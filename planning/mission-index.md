...
| S1L-e | description container/cluster sizing | todo | S1L | 30 fixtures: container box + child-cluster geometry (`computeContainerBbox` subsystem). Largest bucket. Exit: those backlog entries → ≤0.01in, deleted |
| S1L-f | description sprite/stdlib-macro/icon sizing | todo | S1L | 13 fixtures: scaled sprite dims via `!include <awslib\|c4\|…>` macros (kofuca-08-pafi749 → 478in), unknown sprite/icon → 0 dims. Exit: those backlog entries → ≤0.01in, deleted |
