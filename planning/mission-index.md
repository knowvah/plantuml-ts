...
| S1L-e | description container/cluster sizing | todo | S1L | **47 fixtures (dominant bucket)**: container box + child-cluster geometry (`computeContainerBbox` subsystem). e.g. fepuvo-06-rugi981, tuliba-37-liza126. Exit: those backlog entries → ≤0.01in, deleted. `npx tsx scripts/measure-description-size-deltas.ts` |
...
| S1L-k | sprite ink-bounds footprint in use-case ellipses | todo | S1L-f | 2 fixtures (ruziru-69-xixo434, bootstrap-0, 0.124042 each). A sprite's USE-CASE footprint is not always its declared box: jar-measured, `bi-globe` at scale 2.5 is **43.077 in a rectangle but 39.642 in a usecase**, while `bi-bootstrap-fill` is 43.077 in BOTH — identical `<svg width="16" height="16">`, identical scale, both scale syntaxes agreeing. So the ellipse path uses something content-dependent, most plausibly `SvgNanoParser`'s drawn-ink bounds rather than the declared box (an SVG path-bbox port). Size that subsystem before committing to it. Exit: those backlog entries → ≤0.01in, deleted || S1L-c | description interface `hideText` sizing | done | S1L | **DONE 2026-07-28.** The bucket's premise was half right: the fix is not the *shield* but `hideText` — `EntityImageDescription.java:137` (`hideText = symbol == USymbols.INTERFACE`) builds `asSmall` from EMPTY name/desc/stereo (`:209-211`), so an `interface`/`circle` leaf measures the bare `CircleInterface2` square (radius 8 + margin 1 = 18px = **0.25in**) regardless of its label; the label is drawn OUTSIDE and the shield, when not suppressed, reserves room via HTML-table margins instead of growing the node. Ported as `INTERFACE_CIRCLE_SIZE` in `leaf-sizing.ts#measureLeafNode`. This UNMASKED and also closed a pre-existing parser gap: a bare quoted `"Display" as Alias` (no keyword) is `CommandCreateElementFull`'s DISPLAY2/CODE2 branch with SYMBOL omitted → `actorStyle().toUSymbol()` (java:272-275), which makes `isUsecase()` true so every STILL_UNKNOWN leaf mutes to actor; `RE_BARE_QUOTED_DECL` admitted only the no-alias form, widening xacaxe-43 0.556→1.296 until the `as` clause was added. **239 → 272/351 (68.1% → 77.5%) conformant, zero widened, 33 pins deleted**; structure unmoved (component 262/262, usecase 90/90). Bucket now EMPTY — the 2 non-flipped fixtures were classifier misattributions, corrected in the same commit (turasu-73 → S1L-f sprite; cukafa-49 → new S1L-h). See `plans/s1l-leaf-sizing/ledger.md` § S1L-c |
| S1L-h | description per-element font skinparam sizing | todo | S1L | 2 fixtures (cukafa-49-fona812, gogamo-72-pibo470): `skinparam <element>FontSize/FontName/FontStyle` never reaches `measureLeafNode` — `fontSpec` is a single diagram-wide value threaded down `layout-dot-tree.ts`, with no per-USymbol resolution (cukafa-49's components measure 117×44 vs the jar's 139×48 under `componentFontSize 18`). Mirror S1L-g's `resolveElementMinimumWidth` seam, which already threads a per-element value per USymbol through ClassifyCtx→measureBox; the renderer side must move with the sizer or the two diverge. Exit: those backlog entries → ≤0.01in, deleted. `npx tsx scripts/measure-description-size-deltas.ts` |
