...
| S1L-f | description sprite/stdlib-macro/icon sizing | wip | S1L | 12 fixtures (turasu-73-zoni468 joined from the corrected classifier — see S1L-c). **Part 1 DONE 2026-07-28:** the 478in outlier was NOT sprite sizing but **engine misrouting**. `skinparam <selector><<stereo>> {` matched neither block-open regex, fell through to `RE_SKINPARAM_LINE` (key `rectangle<<stereo>>`, value `{`), so the opener was eaten and the block BODY leaked as diagram content. Every awslib/tupadr3 sprite include ends in an `AWSEntityColoring(x)` `!definelong` expanding to exactly that block, so kofuca-08 carried 53 orphan lines ahead of its content — burying the element declarations past `descriptive-keywords.ts`'s `SCAN_LINE_LIMIT`, so `hasDescriptiveElement` said false and `dispatcher.ts#resolve` fell back to the detected type, handing an AWS deployment diagram to the CLASS engine (which measured the raw `<img data:…base64…>` markup as label text, 19214px vs 142px). Fixed in `preprocessor.ts`; kofuca-08 **478.94 → 0.306in**. **Part 2 REMAINING:** genuine sprite-dimension resolution — bundle sprites contribute no dims (ruziru-69/bootstrap-0: `<$bi-globe>` node measures 23.5×20 vs the jar's 69.5×56.8; `,scale=2.5` unhandled). Exit: those backlog entries → ≤0.01in, deleted |
