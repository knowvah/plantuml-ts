...
| A2s | class record-node sizing (→ size-`conformant`) | todo | A2, S1L | 489 RATCHET fixtures non-conformant on node width/height (median 0.086in; small pervasive width residuals in HTML-record compartment/member-row/stereotype geometry + a heavier multi-member tail). Port class node sizing to match the deterministic oracle; drive `oracle/goldens/class/size-backlog.json` entries to ≤0.01 and delete them. Exit: ≥90% conformant / 100%-minus-named. `npx tsx scripts/measure-class-size-deltas.ts` |
...
| SI10 | retire `measureUsecase`'s class-engine coupling | todo | — | `usecase-footprint.ts` → `footprintBoxes` → `measureUsecase` no longer called unconditionally from the class engine; the analytic substitute retired | **Inherited, not new.** `src/diagrams/class/class-layout-leaf-shapes.ts:14,27` calls `measureUsecase` unconditionally from the CLASS engine, predating the guard mechanism — which is why `svg-sprite-nanoparser`'s ADR-3 scoped it out and made touching it a STOP. Now inherited by whoever next touches `leaf-sizing.ts`'s resolver. See `plans/s1l-leaf-sizing/ledger.md` § "The SVG-sprite ink gap". |
| SI8 | stdlib package registration seam | todo | SI5b (done) | a browser-safe way for a consumer to register which stdlib packages back `!include <thing>`, with per-bundle lazy loading; `renderFixture` wires it and the three sprite goldens revert from inlined declarations to `!include <bootstrap/bootstrap>` | **Raised by the maintainer 2026-07-30.** SI5b delivered *resolution* — `stdlibStore(...bundles)` + `withStdlib(base, stdlib)`, both exported from `src/index.ts`, and `!include <bootstrap/bootstrap>` resolves today. What it did NOT deliver is *registration*: the only builder is `scripts/stdlib-assets-store.ts`, which imports `node:fs` and walks `assets/stdlib/`, so it is a script and cannot ship to a browser. A browser consumer must hand-assemble `BundleData[]` from a package's generated exports. No discovery, no lazy loading — and `@plantuml-ts/stdlib-tupadr3` is 19.6 MB, so eager loading is not an option; the design has to be per-bundle (probably per-*resource*) demand loading behind the existing sync `IncludeStore` seam, which is the hard part: the interpreter is synchronous by design, so the async fetch must happen in a `prefetchIncludes`-style pass, not inside `get()`. **Adjacent bug to fix in the same mission:** `tests/oracle/svg-conformance/render-fixture.ts` says in its own doc comment that it "Mirrors `scripts/svg-conformance-census.ts#renderFixture` … exactly", but the census builds a `censusIncludeStore()` (`svg-conformance-census.ts:148-155`) and the harness passes no options to `buildBlockUmls` at all — so no golden can use `!include`. That drift is why `svg-sprite-nanoparser`'s three sprite fixtures inline their declarations (ADR-5); once the harness wires a store they should revert to the `!include` form, which is what a user actually writes. |
