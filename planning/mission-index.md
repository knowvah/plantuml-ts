...
| S1L-g | description minClassWidth/MinimumWidth floor | wip | S1L | **Skinparam path DONE 2026-07-27:** `skinparam minClassWidth` → leaf-box content-width floor (`BoxSizingOpts.minimumWidth` threaded skinparam→theme→ClassifyCtx→measureBox). dexigu/kenece/zifaji widths now pixel-exact vs oracle; delta 2.37→0.31in, pins tightened. **Remaining:** their 0.31in residual is a bracket-body HEIGHT bug (→ S1L-b, not min-width); zotiru-33's `<style> package { MinimumWidth }` scoped-style path is still unwired (needs per-element style MinimumWidth). Exit: those 4 backlog entries → ≤0.01in, deleted |
| S1L-b | description display-text expansion | todo | S1L | 4 fixtures: expand bracket-body `[…]`, `$var`, `<U+…>`/emoji, creole `====` into text lines in the parser/`finalizeDisplay` layer before `measureLeafNode`. Exit: those backlog entries → ≤0.01in, deleted |
