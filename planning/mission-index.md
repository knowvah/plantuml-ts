...
| S1L-k | sprite ink-bounds footprint in use-case ellipses | wip | S1L-f | **Mechanism SOLVED + ported 2026-07-28.** A use-case ellipse takes only `alpha` from `calculateDimension`; the ellipse itself is fit to `Footprint`'s collected points, and a drawn path contributes just its min/max corners — computed by `UPath#addInternal`, which records ONLY a `SEG_ARCTO`'s endpoint and EVERY other coordinate pair including Bézier control points (so: control-polygon box, no curve flattening). New `svg-path-bbox.ts` + `SpriteSvg.inkWidth/inkHeight` + `spriteInkDimsLookupFor`. That explains bi-globe (inks 16×13.846, outer circle is an arc) vs bi-bootstrap-fill (16×16) from identical `<svg width="16" height="16">`. **5 of ruziru-69's 6 nodes now EXACT**; both fixtures 0.124042 → 0.069047. **Residual (open):** node `a` is a MIXED sprite+text display where the footprint depends on actual DRAW POSITIONS (`Footprint#drawText` offsets by `height − 1.5`; a sprite's ink is offset inside its line box), which a per-line max/sum model cannot express — back-solving did not reconcile, so it is diagnosed-but-open, not guessed. Exit: model `Footprint`'s point collection with draw positions; entries → ≤0.01in, deleted |
