...
beforeAll(() => {
  ...
  // salt: manifest exists, no cache dir at all (and not a NON_SVEK_TYPES
  // member -- board moved into that set once its oracle was captured).
  writeManifest(dataDir, 'salt', [{ slug: 'salt-1', markup: '@startuml\nsalt\n{T\n}\n@enduml\n' }]);
...
describe('NON_SVEK_TYPES', () => {
  it('is exactly sequence, activity, json, yaml, hcl, dot, gitgraph, board, chart, chronology, files, packet', () => {
...
describe('dotParityRows', () => {
  it('returns exactly one row per manifest in roots.dataDir, sorted by type', () => {
    ...
    expect(rows.map((r) => r.type)).toEqual(['alpha', 'class', 'salt', 'sequence', 'zeta']);
  });
  ...
  it('salt (no cache dir, not a NON_SVEK_TYPES member): no oracle captured', () => {
    const row = rowFor(dotParityRows(UNUSED_JAR, roots), 'salt');
...
describe('dotParityMarkdown', () => {
  const rows: TypeRow[] = [
    ...
    { type: 'salt', comparable: 0, equal: 0, pct: '—', oracleBlind: 0, note: 'no oracle captured' },
  ];
  ...
  it('renders the generated-on date, the header, one table row per TypeRow, and the legend', () => {
    ...
    expect(out).toContain('| salt | 0 | 0 | — | 0 | no oracle captured |');
  });
