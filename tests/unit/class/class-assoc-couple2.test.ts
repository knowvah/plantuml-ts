...
describe('association-class couple: subsumed-edge length transfer', () => {
  it(
    'a single-dash (length 1) subsumed self-couple transfers minlen 0 onto ' +
  ...
  it(
    'a double-dash (length 2) subsumed distinct-pair couple transfers ' +
  ...
  it(
    'a single-dash (length 1) subsumed distinct-pair couple flips the ' +
  ...
  it(
    'with NO prior explicit association, the entity<->circle edges default ' +
  ...
  it(
    'an existing link on a PORT endpoint (Class::member) still subsumes by ' +
  ...
  it(
    'a PORT on the B-side endpoint of the subsumed link registers on Bar, ' +
  ...
  it(
    'subsumes the MOST RECENTLY declared relationship between the pair, ' +
    ...
    `);
      ...
      const direct = ast.relationships.find(
        (r) =>
          (r.from === research.id && r.to === correlations.id) || (r.from === correlations.id && r.to === research.id),
      );
      ...
      expect(
        ast.relationships.some(
          (r) =>
            (r.from === research.id && r.to === experiments.id) || (r.from === experiments.id && r.to === research.id),
        ),
      ).toBe(false);
    },
  );
...
describe('association-class couple: note-on-link split', () => {
  ...
  it(
    'note on link: text — SPLIT across both entity<->circle edges when the ' +
...
describe('association-class couple: render-layer decor/dashing (G2 N8)', () => {
  it(
    "the class-link edge (leading form) carries the couple arrow's OWN " +
  ...
  it(
    'entity<->circle edges stay undecorated/solid when the subsumed ' +
  ...
  it(
    'a REPEAT coupling on an already-coupled pair marks the sibling-circle ' +
    ...
    `);
      ...
      const invisEdge = ast.relationships.find((r) => r.invis === true && r.from === circles[0] && r.to === newCircle);
...
describe('association-class couple: (A,B) referencing a note id', () => {
  it(
    'a freestanding note used as the trailing couple target reuses the ' +
...
describe('association-class couple: G2 N19 synthetic-id naming (single coupling)', () => {
  it(
    'names the circle "apointN" and dense-numbers name/uid/edge phantom ' +
  ...
  it(
    "carries the SUBSUMED explicit association's own creationIndex as a " +
  ...
  it(
    'G2 N20: stamps BOTH circles of a repeat-coupled (A,B) pair -- the ' +
  ...
  it(
    'G2 N20: the CONDITIONAL getInv() inversion fires when the PRIOR ' +
    ...
    `);
      ...
      const order = ast.relationships.filter((r) => r.invis !== true).map((r) => `${r.from}->${r.to}`);
