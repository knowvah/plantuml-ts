...
describe('buildClassMagmaEdges — standalone-leaf invisible chaining', () => {
  it('chains 6 disconnected leaves into a 2×3 grid (5 invisible edges)', () => {
    ...
    const key = (e: (typeof edges)[number]) => `${e.from}->${e.to}:${e.attributes!.minLen}`;
    expect(edges.map(key).sort()).toEqual(['A->B:0', 'A->D:1', 'B->C:0', 'D->E:0', 'E->F:0'].sort());
  ...
  it('excludes leaves touched by a relationship', () => {
    const a = ast({
      ...
      relationships: [{ from: 'A', to: 'B', type: 'association' } as never],
    });
