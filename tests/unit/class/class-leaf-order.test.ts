...
describe('computeLeafDrawOrder (T2, leaf-draw-order)', () => {
  it(
    'a namespace group draws before unpackaged siblings — jar-verified ' +
  ...
  it("a group's own leaves precede its subgroups (`printGroup`): P{ Q{B} A } " + "draws P's own A before Q's B", () => {
  ...
  it(
    'a hand-built AST with no creationIndex anywhere keeps declaration ' +
  ...
  it(
    'a SECOND member-tip on the same (host, side) has no ' +
  ...
  it(
    'a hand-built member-tip note with no tipGroupPhantomIndex and no ' +
  ...
  it(
    'T5: a ROOT-level empty package sorts as a GROUP sibling of the root ' +
  ...
  it(
    'T5: a NESTED empty package sorts as a child-group sibling of its ' +
  ...
  it(
    'option (b) marker: a `<<Database>>` empty package collapses to a ' +
  ...
  it('the result is always a permutation of every classifier id and every ' + 'note id, with no duplicates', () => {
    const ast = parse('class X\npackage P {\nclass A\nnote "n" as N\n}\nclass Y\nnote left of X : hello');
    const order = computeLeafDrawOrder(ast);
    const expectedIds = new Set([...ast.classifiers.map((c) => c.id), ...ast.notes.map((n) => n.id)]);
