...
import { subdivide, clipSplineStart, clipSplineEnd, type ClipRect as Bbox } from '../../../src/core/spline-clip.js';
...
describe('subdivide (XCubicCurve2D#subdivide port, t=0.5)', () => {
  ...
  test('midpoint equals the analytic Bernstein evaluation at t=0.5', () => {
    ...
    const t = 0.5,
      mt = 0.5;
    const bx = mt * mt * mt * p0.x + 3 * mt * mt * t * c1.x + 3 * mt * t * t * c2.x + t * t * t * p3.x;
    const by = mt * mt * mt * p0.y + 3 * mt * mt * t * c1.y + 3 * mt * t * t * c2.y + t * t * t * p3.y;
...
describe('clipSplineStart (simulateCompound tail branch)', () => {
  ...
  test('drops fully-inside leading segments and keeps the outside tail', () => {
    const pts: Pt[] = [
      ...
      { x: 40, y: 50 },
      { x: 80, y: 50 },
      { x: 150, y: 50 }, // seg1 -> P1 OUT
      { x: 160, y: 50 },
      { x: 170, y: 50 },
      { x: 180, y: 50 }, // seg2 -> P2 out
    ];
    const out = clipSplineStart(pts, tail);
    expect(isValidSpline(out)).toBe(true);
    expect(contains(out[0]!, tail)).toBe(false);
...
describe('F1 regression: cross-container edges no longer dropped', () => {
  ...
  test.each(['berufi-69-dara369', 'lirebi-26-voka556'])('%s renders every edge (zero drops)', (slug) => {
    const puml = readFileSync(`oracle/goldens/description/${slug}/input.puml`, 'utf8');
    let drops = 0;
