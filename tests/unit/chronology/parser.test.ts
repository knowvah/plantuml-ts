...
describe('parseChronology', () => {
  ...
  it('AC2: [Event2] happens at 2023-11-28 14:11:50.750 → timestampMs 1701180710750', () => {
    const ast = parseChronology(makeSource(['[Event2] happens at 2023-11-28 14:11:50.750']));
    expect(ast.events.length).toBe(1);
    expect(ast.events[0]).toEqual({ name: 'Event2', timestampMs: 1701180710750 });
  });

  it('AC3a: "happens on" parses identically to "happens at"', () => {
    const atAst = parseChronology(makeSource(['[A] happens at 2023-01-01 00:00:00']));
    const onAst = parseChronology(makeSource(['[A] happens on 2023-01-01 00:00:00']));
    expect(onAst.events[0]).toEqual(atAst.events[0]);
  });

  it('AC3b: "happens the" parses identically to "happens at"', () => {
    const atAst = parseChronology(makeSource(['[A] happens at 2023-01-01 00:00:00']));
    const theAst = parseChronology(makeSource(['[A] happens the 2023-01-01 00:00:00']));
    expect(theAst.events[0]).toEqual(atAst.events[0]);
  });

  it('AC4: no milliseconds → ms=0, valid UTC epoch value', () => {
    const ast = parseChronology(makeSource(['[A] happens at 2023-01-01 12:00:00']));
    expect(ast.events.length).toBe(1);
  ...
  it('AC5: @startchronology / @endchronology wrapper lines produce no events', () => {
    const ast = parseChronology(makeSource(['@startchronology', '@endchronology']));
    expect(ast.events).toEqual([]);
  });

  it('AC6a: blank lines are silently ignored, no throw', () => {
    const ast = parseChronology(makeSource(['', '[A] happens at 2023-01-01 00:00:00', '']));
    expect(ast.events.length).toBe(1);
  });

  it('AC6b: unrecognized lines are silently ignored, no throw', () => {
    expect(() => parseChronology(makeSource(['this is garbage', 'not a command']))).not.toThrow();
    const ast = parseChronology(makeSource(['this is garbage', '[A] happens at 2023-01-01 00:00:00']));
    expect(ast.events.length).toBe(1);
  });
  ...
  it('event name with spaces and special chars is preserved verbatim', () => {
    const ast = parseChronology(makeSource(['[My Event Name] happens at 2023-06-15 08:30:00']));
  ...
  it('milliseconds with trailing digits uses full numeric value', () => {
    // .750 → ms=750
    const ast = parseChronology(makeSource(['[E] happens at 2023-01-01 00:00:00.750']));
  ...
  it('@STARTCHRONOLOGY uppercase wrapper is also stripped', () => {
    const ast = parseChronology(
      makeSource(['@STARTCHRONOLOGY', '[A] happens at 2023-01-01 00:00:00', '@ENDCHRONOLOGY']),
    );
    expect(ast.events.length).toBe(1);
