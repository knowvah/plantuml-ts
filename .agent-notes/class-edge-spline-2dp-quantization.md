...
Inside **Smetana's layout**, not at PlantUML's read seam and not in our
port. Narrowed by elimination:

- `sdot/SmetanaEdge.java#getPoint` reads `ST_pointf` straight through
  (`new XPoint2D(pt.x, pt.y)`) — no rounding there.
- `h/ST_pointf.java` declares `public double x/y` — not `float`, so this
  is not float-precision truncation.
- The jar's own node rect and its own spline disagree: node
  `x=7 width=49.15` puts the centre at **31.575**, yet the jar draws the
  vertical edge at **31.58**. A vertical edge's x must be the node centre,
  so the rounding happens between layout and that edge's coordinates.
